...
# TODO: Add time to all the functions. Disturbance could be added internally
...
#         xx[i+1, :] = next_state(plant, xx[i+1, :], uu[i, :])
#         zz[i+1, :] = next_state(ctrl, zz[i, :], yy[i, :])
#     end
# end

function simulate(sys::DiscreteSystem, x0, input)
    x = x0
    for u in input
        x = next_state(sys, x, u)
    end
    x
end