@testset "Basic expression tests" begin
    ...
    @testset "MLiteral tests" begin
        ...
        @test symbols(lit1) == Set([])
        @test is_constant(lit1, t)
        @test is_constant(lit1, s)
    end

    @testset "MSymbols tests" begin
        @test symbols(t) == Set([t])
        @test symbols(s) == Set([s])
    ...
    @testset "Summation tests" begin
        ...
        @test (t + lit1) == (t + 1)
        @test (lit1 + t) == (1 + t)
        @test symbols(1 + t) == Set([t])
        @test symbols(t + 1) == Set([t])
        @test symbols(t + t) == Set([t])
        @test symbols(t + t + s) == Set([t, s])
        @test symbols((t + 1) + (t + s)) == Set([t, s])
    end
